...
# I will find export async function cancelarReserva(reservaId: string) { ... }
# since it ends right before `export async function rechazarSolicitud`
...
cancelar_reserva_new = """export async function cancelarReserva(reservaId: string) {
  try {
    ...
    await db.from('fechas_bloqueadas').delete().eq('reserva_id', reservaId).catch(() => {})

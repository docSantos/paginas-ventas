...
# 5. We need to close the `<div className="space-y-6">` of the directorio tab AND the expression `)}`.
# The very last part of the file is:
#       </Dialog>
#     </div>
#   )
# }
pattern_end = re.compile(r'(</Dialog>\s*</div>\s*\)\s*\})', re.DOTALL)
content = pattern_end.sub(r'</Dialog>\n        </div>\n      )}\n    </div>\n  )\n}', content)

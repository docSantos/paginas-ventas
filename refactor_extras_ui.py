...
# 1. State changes:
# We need to change `const [selectedExtras, setSelectedExtras] = useState<Record<string, number>>({})`
# to `const [selectedExtras, setSelectedExtras] = useState<Record<string, { activo: boolean, qty: number, ida?: boolean, vuelta?: boolean }>>({})`
...
old_cotizacion = r"let extrasTotal = 0;\s*Object\.keys\(selectedExtras\)\.forEach\(servId => \{.*?\n\s*\}\);"
...
# Find "Fechas ocupadas" block and extract it
fechas_block_regex = r"(\{\s*reservas\.length > 0 && \(\s*<div className=\"mb-4 p-3 bg-red-50 text-red-800 text-sm rounded-xl border border-red-100\">\s*<p className=\"font-semibold mb-1 flex items-center gap-1\"><AlertCircle className=\"w-4 h-4\" /> Fechas ocupadas:</p>\s*<ul className=\"list-disc pl-5 space-y-0\.5 text-xs\">\s*\{reservas\.map\(\(r, i\) => \(\s*<li key=\{i\}>\{formatDateEs\(r\.fecha_entrada\)\} al \{formatDateEs\(r\.fecha_salida\)\}<\/li>\s*\)\)\}\s*<\/ul>\s*<\/div>\s*\)\s*\})"

match_fechas = re.search(fechas_block_regex, content)
if match_fechas:
    fechas_str = match_fechas.group(1)
    content = content.replace(fechas_str, "")
    
    # Insert it before "Cantidad de personas"
    insert_point = """          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block">
              Cantidad de personas
            </label>"""
    
    content = content.replace(insert_point, fechas_str + "\n\n" + insert_point)

# 6. Replace UI for services
old_services_ui = r"\{servicios && servicios\.length > 0 && \(\s*<div className=\"mb-4 pt-4 border-t border-gray-200\">\s*<label className=\"text-sm font-medium text-gray-700 mb-2 block\">\s*Personaliza tu estancia con servicios extra\s*<\/label>\s*<div className=\"space-y-2\">\s*\{servicios\.map\(\(serv: any\) => \{\s*const isSelected = selectedExtras\[serv\.id\] > 0;\s*return \(\s*<div key=\{serv\.id\} className=\{\`p-3 border rounded-xl flex flex-col gap-2 transition-colors \$\{isSelected \? 'bg-teal-50 border-teal-200' : 'bg-white border-gray-200'\}\`\}>\s*<div className=\"flex items-start gap-3\">\s*<input\s*type=\"checkbox\"\s*className=\"mt-1 rounded text-teal-600 focus:ring-teal-500\"\s*checked=\{isSelected\}\s*onChange=\{\(e\) => \{\s*setSelectedExtras\(prev => \(\{\s*\.\.\.prev,\s*\[serv\.id\]: e\.target\.checked \? 1 : 0\s*\}\)\)\s*\}\}\s*\/>\s*<div className=\"flex-1\">\s*<p className=\"text-sm font-medium text-gray-900\">\{serv\.nombre\}<\/p>\s*<p className=\"text-xs text-gray-500\">\{serv\.descripcion\}<\/p>\s*<\/div>\s*<div className=\"text-right\">\s*<p className=\"text-sm font-semibold text-teal-700\">\+\{formatPrice\(serv\.precio_base\)\}<\/p>\s*<p className=\"text-\[10px\] text-gray-400\">\{serv\.tipo_tarifa === 'fijo' \? 'Total' : 'c\/u'\}<\/p>\s*<\/div>\s*<\/div>\s*\{isSelected && serv\.tipo_tarifa === 'por_noche' && \(\s*<div className=\"ml-7 flex items-center gap-3\">\s*<span className=\"text-xs text-gray-600\">Cantidad:<\/span>\s*<div className=\"flex items-center gap-2\">\s*<button className=\"w-6 h-6 rounded-md border border-gray-300 flex items-center justify-center bg-white text-gray-600 text-sm\" onClick=\{\(\) => setSelectedExtras\(prev => \(\{\.\.\.prev, \[serv\.id\]: Math\.max\(1, \(prev\[serv\.id\] \|\| 1\) - 1\)\}\)\)\}>-<\/button>\s*<span className=\"text-sm font-medium w-4 text-center\">\{selectedExtras\[serv\.id\]\}<\/span>\s*<button className=\"w-6 h-6 rounded-md border border-gray-300 flex items-center justify-center bg-white text-gray-600 text-sm\" onClick=\{\(\) => setSelectedExtras\(prev => \(\{\.\.\.prev, \[serv\.id\]: \(prev\[serv\.id\] \|\| 1\) \+ 1\}\)\)\}>\+<\/button>\s*<\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\)\s*\}\)\}\s*<\/div>\s*<\/div>\s*\)\}"
                    ...
                    <div key={serv.id} className={`p-3 border rounded-xl flex flex-col gap-3 transition-colors ${isSelected ? 'bg-teal-50 border-teal-200' : 'bg-white border-gray-200'}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500">{serv.tipo_tarifa === 'por_dia' ? 'Por da' : (serv.tipo_tarifa === 'por_trayecto' ? 'Por trayecto' : 'Pago nico')}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-teal-700">+{formatPrice(serv.precio_base)}</p>
                          <p className="text-[10px] text-gray-400">
                            {serv.tipo_tarifa === 'por_dia' ? 'x da' : (serv.tipo_tarifa === 'por_trayecto' ? 'c/u' : 'Total')}
                          </p>
                        </div>
                      </div>
                      ...
                      {isSelected && serv.tipo_tarifa === 'por_dia' && (
                        <div className="ml-7 flex items-center justify-between gap-3 bg-white p-2 rounded-lg border border-teal-100">
                          <div className="flex items-center gap-3">
                            <span className="text-xs font-medium text-gray-700">Das de renta:</span>
...
content = re.sub(old_services_ui, new_services_ui, content, flags=re.DOTALL)

with open('src/components/casasgaby/PropertyDetailClient.tsx', 'w', encoding='utf-8') as f:
    f.write(content.replace("da", "día").replace("nico", "único"))

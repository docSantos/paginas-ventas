...
# 2. Add CrmPipeline imports and states (I will define it at the bottom)
...
# Insert right after function opening
...
# We need to find the `return (` of the component.
pattern_return = re.compile(r'return \(\s*<div className="space-y-6">')
return_match = pattern_return.search(content)
...
if return_match:
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'directorio' && (
        <div className="space-y-6">
"""
    content = content[:return_match.start()] + new_return + content[return_match.end():]

# 5. We need to close the `<div className="space-y-6">` of the directorio tab.
# We'll just add `</div>` before the final `</Dialog>\n    </div>\n  )\n}`
pattern_end = re.compile(r'(</Dialog>\s*</div>\s*\)\s*\})', re.DOTALL)
content = pattern_end.sub(r'</div>\n      \1', content)

# 6. Append the CrmPipeline Component at the end of the file
crm_pipeline_code = """
function CrmPipeline({ solicitudes }: { solicitudes: any[] }) {
  const router = useRouter()
  // Add required imports for icons if not already there, we can just use the ones available.
  const stages = [
  ...
  // If a solicitud is "Pendiente", treat it as "nueva".
  ...
  const renderCards = (stageId: string) => {
    ...
    return items.map(s => (
      <div key={s.id} className="bg-white p-3 rounded-lg border shadow-sm flex flex-col gap-2 relative">
        ...
        <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t">
          <a href={buildWaUrl(s.codigo_pais || '+52', s.telefono, `Hola ${s.nombre_cliente}, te escribo de Casas Gaby.`)} target="_blank" className="flex-1 bg-green-50 text-green-700 border border-green-200 text-xs py-1 rounded-md text-center hover:bg-green-100 font-medium transition-colors">
            WhatsApp
          </a>
          <select 
            className="flex-1 text-xs border rounded-md px-1 py-1 bg-gray-50 focus:ring-1 focus:ring-teal-500"
            value={s.estado_crm}
        ...
        {(stageId !== 'convertida' && stageId !== 'descartada') && (
          <Button size="sm" className="w-full mt-1 bg-teal-600 hover:bg-teal-700 text-white h-7 text-xs" onClick={() => window.location.href='/casasgaby/admin/reservas'}>
            Aprobar / Convertir
          </Button>
        )}
      </div>
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
          {stages.map(st => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${activeStage === st.id ? st.color + ' ring-1 ring-black/10' : 'bg-white text-gray-600 border-gray-200'}`}
            >
              {st.title} ({normalizedSolicitudes.filter(s => s.estado_crm.toLowerCase() === st.id.toLowerCase()).length})
            </button>
          ))}
        </div>
        <div className="bg-gray-50 rounded-xl p-3 min-h-[400px] border border-gray-100">
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-6 min-h-[600px]">
      {stages.map(st => (
        <div key={st.id} className="flex-none w-72 flex flex-col bg-gray-50/50 rounded-xl border border-gray-200/60 overflow-hidden">
          <div className={`p-3 border-b border-black/5 font-bold text-sm flex justify-between items-center ${st.color}`}>
            <span>{st.title}</span>
            <span className="bg-white/50 px-2 py-0.5 rounded-full text-xs">{normalizedSolicitudes.filter(s => s.estado_crm.toLowerCase() === st.id.toLowerCase()).length}</span>
          </div>

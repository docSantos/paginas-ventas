...
# Update logic
old_logic = r"const addNumber = async \(\) => \{[\s\S]*?const fullNumber = `\$\{lada\}\$\{cleanNumber\}`"
...
old_input = r"<div className=\"flex\">\s*<select[\s\S]*?</select>\s*<Input[\s\S]*?/>\s*</div>"
new_input = r"<PhoneInputField value={nuevoNumero} onChange={setNuevoNumero} />"
...
# Update error msg
old_err = r"\{nuevoNumero\.length > 0 && !isPhoneValid\(nuevoNumero, lada\) && \(\s*<p className=\"text-xs text-red-500 mt-2\">Ingresa un n.mero v.lido \(\{lada === '52' \|\| lada === '1' \? '10' : '8-15'\} d.gitos\)</p>\s*\)\}"

...
# 3. Modify Cancel Button logic via Regex
...
old_btn_pattern = r"<Button\s+size=\"sm\"\s+variant=\"outline\"\s+className=\"text-red-600 border-red-200 hover:bg-red-50 ml-auto\"\s+onClick=\{async \(\) => \{\s+if \(confirm[^>]+>\s+Cancelar Reserva\s+</Button>"
...
content = re.sub(old_btn_pattern, new_cancel_btn, content, flags=re.DOTALL)

...
# Add imports
if "crearServicio" not in content:
    content = content.replace("import PhoneInputField from '@/components/PhoneInputField'", "import PhoneInputField from '@/components/PhoneInputField'\nimport { crearServicio, actualizarServicio, eliminarServicio } from '@/app/casasgaby/admin/actions'\nimport { Edit2, Trash2 } from 'lucide-react'\nimport { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog'\nimport { formatPrice } from '@/lib/utils'")
...
"""
if "loadServicios" not in content:
    content = content.replace("const loadConfig = async () => {", state_code + "\n  const loadConfig = async () => {")
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-purple-50 p-3 rounded-lg border border-purple-100 mt-2">
              <span className="text-sm font-medium text-purple-900 block mb-0.5">Comisión de servicio automatizada</span>
              <p className="text-xs text-purple-700">Se heredará el 5% del tenant.</p>
            </div>
...
"""
if "SECCIÓN SERVICIOS ESPECIALES" not in content:

...
content = content.replace(
    "export function ReservasClient({ solicitudes, reservas, servicios = [], tenantExtras = 5 }: { solicitudes: Solicitud[], reservas: Reserva[], servicios?: any[], tenantExtras?: number }) {",
    "export function ReservasClient({ solicitudes, reservas, servicios = [], tenantExtras = 5, tenantBase = 2.50 }: { solicitudes: Solicitud[], reservas: Reserva[], servicios?: any[], tenantExtras?: number, tenantBase?: number }) {"
)

# Add state for selected extras in Aprobar Modal
state_mods = """  const [aprobarModal, setAprobarModal] = useState<{ open: boolean, solicitud: any | null }>({ open: false, solicitud: null })
  ...
  const [selectedExtras, setSelectedExtras] = useState<Record<string, { qty: number, monto: number, pct: number, concepto: string }>>({})"""

content = re.sub(
    r"const \[aprobarModal, setAprobarModal\].*?const \[tc, setTc\] = useState\('20\.00'\)",
    state_mods,
    content,
    flags=re.DOTALL
)

# Open aprobar modal -> reset extras
open_aprobar = """const openAprobar = (solicitud: any) => {
...
content = re.sub(
    r"setAprobarModal\(\{ open: true, solicitud: s \}\)\s*setMontoAcordado\(String\(s\.costo_total \|\| ''\)\)",
    open_aprobar,
    content,
    flags=re.DOTALL
)

# Replace the onClick calls to openAprobar
content = content.replace(
    "onClick={() => {\n                          setAprobarModal({ open: true, solicitud: s })\n                          setMontoAcordado(String(s.costo_total || ''))\n                        }}",
    "onClick={() => openAprobar(s)}"
)

# handleAprobar function update
handle_aprobar = """  const handleAprobar = async () => {
...
content = re.sub(
    r"const handleAprobar = async \(\) => \{.*?setAprobarModal\(\{ open: false, solicitud: null \}\)\n    \} catch \(e: any\) \{\n      alert\(\"Error: \" \+ e\.message\)\n    \}\n  \}",
    handle_aprobar,
    content,
    flags=re.DOTALL
)
...
"""
content = re.sub(
    r"\{\/\* Modal Aprobar \*\/\}.*?<DialogTitle>Aprobar Reserva</DialogTitle>.*?<CheckCircle className=\"w-4 h-4 mr-2\" /> Aprobar\s*</Button>\s*</div>\s*</div>\s*</DialogContent>\s*</Dialog>",
    modal_ui,
    content,
    flags=re.DOTALL
)

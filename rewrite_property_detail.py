...
# Search for `let extrasTotal = 0;` and replace until `    });`
import re
...
# We use split to slice the text exactly.
...
# Find "{servicios && servicios.length > 0 && ("
parts = content.split('{servicios && servicios.length > 0 && (')
if len(parts) == 2:
    before_servicios = parts[0]
    rest = parts[1]
    
    parts_2 = rest.split('          {cotizacion && !errorFechas && (')
    
    # But wait, now `cotizacion` is right after `servicios` because we moved `fechas ocupadas`!
    # Or maybe it's right after `fechas_block` if we moved it up... wait. We moved it up!
    # So `servicios` is followed by `cotizacion`.
    # Actually, `fechas_block` was originally after `servicios`.

...
"""
    ...
    content = content.replace("export function FinanzasClient({ propiedades, reservas, pagos }: { propiedades: any[], reservas: any[], pagos: any[] }) {", 
    "export function FinanzasClient({ propiedades, reservas, pagos }: { propiedades: any[], reservas: any[], pagos: any[] }) {" + helper)
...
# We will use simple replace for each one to avoid regex issues
# Meses Pasados
...
# Total del Año - check for "Ao" due to encoding
content = re.sub(
    r'<CardContent className="pt-4">\s*<div className="text-sm text-gray-600 font-bold mb-1">Total del A.o</div>\s*<div className="text-xl font-black text-gray-900">\{formatPrice\(metrics\.totalYearLost\)\}</div>',
